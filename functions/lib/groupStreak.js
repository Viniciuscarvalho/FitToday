...
exports.sendAtRiskNotifications = exports.createWeeklyStreakWeek = exports.evaluateGroupStreaks = void 0;
 ...
 */
exports.evaluateGroupStreaks = functions.pubsub
    .schedule("59 23 * * 0")
    .timeZone("UTC")
    .onRun(async () => {
    ...
    try {
        ...
        for (const groupDoc of groupsSnapshot.docs) {
            ...
            const allCompliant = Object.values(memberCompliance).every((member) => member.workoutCount >= REQUIRED_WORKOUTS);
            ...
            if (allCompliant) {
                ...
                console.log(`[evaluateGroupStreaks] Group ${groupId}: streak incremented to ${newStreakDays} days`);
                // Send milestone notification if applicable
                if (achievedMilestone) {
                    notifications.push(sendMilestoneNotification(groupId, groupData.name, achievedMilestone));
                }
            }
            else {
                // Reset streak
                ...
                if (currentStreakDays > 0) {
                    notifications.push(sendStreakBrokenNotification(groupId, groupData.name, currentStreakDays));
                }
            }
        }
        // Commit all updates
 ...
 */
exports.createWeeklyStreakWeek = functions.pubsub
    .schedule("0 0 * * 1")
    .timeZone("UTC")
    .onRun(async () => {
    ...
    try {
        ...
        for (const groupDoc of groupsSnapshot.docs) {
            ...
            console.log(`[createWeeklyStreakWeek] Created week record for group ${groupId} with ${membersSnapshot.size} members`);
        }
        // Commit all records
        await batch.commit();
 ...
 */
exports.sendAtRiskNotifications = functions.pubsub
    .schedule("0 18 * * 4")
    .timeZone("UTC")
    .onRun(async () => {
    ...
    try {
        ...
        for (const groupDoc of groupsSnapshot.docs) {
            ...
            const weekData = weekSnapshot.docs[0].data();
            const memberCompliance = weekData.memberCompliance || {};
            ...
            console.log(`[sendAtRiskNotifications] Group ${groupId}: ${atRiskMembers.length} at-risk members`);
            // Send individual notifications to at-risk members
            for (const member of atRiskMembers) {
                notifications.push(sendIndividualAtRiskNotification(member.userId, groupId, groupData.name, member.workoutsRemaining, streakData.groupStreakDays));
            }
            // Send group notification about at-risk members
            notifications.push(sendGroupAtRiskNotification(groupId, groupData.name, atRiskMembers));
        }
        await Promise.all(notifications);
 ...
 */
async function resetMonthlyPauseFlags() {
 ...
 */
async function sendMilestoneNotification(groupId, groupName, milestone) {
 ...
 */
async function sendStreakBrokenNotification(groupId, groupName, previousDays) {
 ...
 */
async function sendIndividualAtRiskNotification(userId, groupId, groupName, workoutsRemaining, streakDays) {
 ...
 */
async function sendGroupAtRiskNotification(groupId, groupName, atRiskMembers) {
 ...
 */
async function createGroupNotification(groupId, type, message) {
