...
final class DailyQuestionnaireViewModelTests: XCTestCase {
  ...
  var mockEntitlementRepo: MockEntitlementRepository!
  var mockProfileRepo: MockUserProfileRepository!
  var mockBlocksRepo: MockWorkoutBlocksRepository!
  var mockComposer: MockWorkoutPlanComposer!
  
  override func setUp() {
    super.setUp()
    mockEntitlementRepo = MockEntitlementRepository()
    mockProfileRepo = MockUserProfileRepository()
    mockBlocksRepo = MockWorkoutBlocksRepository()
    mockComposer = MockWorkoutPlanComposer()
...
class MockEntitlementRepository: EntitlementRepository {
  ...
  func validatePurchase(_ productId: String) async throws -> Bool {
    return false
  }
...
class MockWorkoutBlocksRepository: WorkoutBlocksRepository {
  var mockBlocks: [WorkoutBlock] = []
  
  func load() async throws -> [WorkoutBlock] {
    return mockBlocks
  }
}

class MockWorkoutPlanComposer: WorkoutPlanComposing {
