...
FAIL: Research failed — no research tools available
  ...
  Try again later or manually add research to captures/
```
...
**Write to** `captures/`. Create folder if missing.
...
Format for downstream extract — findings as clear propositions, not raw dumps:
...
should be a clear proposition the extract phase can pull out as an atomic insight.]
...
Research complete
  ...
  Filed to: captures/[filename]

  Next: /extract captures/[filename]
```

Append based on mode:
- **manual:** (nothing extra)
- **suggested:** `Ready for processing when you are.`
- **automatic:** Replace "Next" line with `Queued for /extract -- processing will begin automatically.`
...
| Phase | Skill | Purpose |
|-------|-------|---------|
| Extract insights | /extract | Mine research for atomic patterns |
| Find connections | /connect | Link new insights to existing graph |
| Update old patterns | /update | Backward pass on touched patterns |
