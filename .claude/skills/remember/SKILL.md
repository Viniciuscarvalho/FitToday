...
| connection | How to find, evaluate, or add links between patterns |
...
| quality | Standards for patterns, descriptions, titles |
...
- Use concrete examples where possible — "when processing iOS session notes" not "when processing certain types of content"
...
|------------|-------------------|---------|
| Direct correction | "no", "that's wrong", "not like that", "incorrect" | "No, don't split that into separate patterns" |
...
- The title should be an actionable behavior ("check for semantic duplicates before creating any pattern") not a problem description ("duplicate creation issue").
...
- ops/methodology/ is consulted by meta-skills (/arscontexta:ask, /arscontexta:architect, /rethink) as the source of truth for how the system works.
...
|------------------------|--------------------------|
| "duplicate creation issue" | "check for semantic duplicates before creating any pattern" |
...
| Category | Use When |
|----------|---------|
| processing | Friction during /extract, extraction, pattern creation |
| capture | Friction during inbox filing, raw material handling |
| connection | Friction during /connect, link evaluation, domain guide updates |
...
| quality | Friction about pattern quality, description writing, title crafting |
...
Create it and the `ops/methodology/methodology.md` MOC:
...
- User manually edits a pattern the agent created (the edit reveals what was wrong)
