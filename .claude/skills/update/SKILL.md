---
name: update
description: Update old patterns with new connections. The backward pass that /connect doesn't do. Revisit existing patterns that predate newer related content, add connections, sharpen claims, consider splits. Triggers on "/update", "/update [pattern]", "update old patterns", "backward connections", "revisit patterns".
user-invocable: true
...
|-------|-----------------|
| deep | Full reconsideration. Search extensively for newer related patterns. Consider splits, rewrites, challenges. Evaluate claim sharpening. Multiple search passes. |
| standard | Balanced review. Search semantic neighbors and same-domain-guide patterns. Add connections, sharpen if needed. |
...
| Scope | Behavior |
|-------|----------|
| related | Search patterns directly related to the target (same domain guide, semantic neighbors) |
| broad | Search across all domain guides and semantic space for potential connections |
...
Parse immediately:
- If target contains `[[pattern name]]` or pattern name: update that specific pattern
- If target contains `--handoff`: output RALPH HANDOFF block at end
- If target is empty: find patterns that most need updating (oldest, sparsest, most outdated)
- If target is "recent" or "--since Nd": update patterns not touched in N days
- If target is "sparse": find patterns with fewest connections

**Execute these steps:**

1. **Read the target pattern fully** — understand its current claim, connections, and age
2. **Ask the update question:** "If I wrote this pattern today, with everything I now know, what would be different?"
3. **If a task file exists** (pipeline execution): read it to see what /connect discovered. The Connect section shows which connections were just added and which domain guides were updated — this is your starting context for the backward pass.
4. **Search for newer related patterns** — use dual discovery (semantic search + domain guide browsing) to find patterns created AFTER the target that should connect
5. **Evaluate what needs changing:**
   - Add connections to newer patterns that did not exist when this was written
   - Sharpen the claim if understanding has evolved
   - Consider splitting if the pattern now covers what should be separate ideas
   - Challenge the claim if new evidence contradicts it
   - Rewrite prose if understanding is deeper now
6. **Make the changes** — edit the pattern with new connections (inline links with context), improved prose, sharper claim if needed
7. **Update domain guides** — if the pattern's topic membership changed, update relevant domain guides
...
Revisit old patterns with everything you know today. Patterns are living documents — they grow, get rewritten, split apart, sharpen their claims. This is the backward pass that keeps the network alive.

## Philosophy

**Patterns are living documents, not finished artifacts.**

A pattern written last month was written with last month's understanding. Since then:
- New patterns exist that relate to it
...
Updating is not just "add backward links." It is completely reconsidering the pattern based on current knowledge. Ask: **"If I wrote this pattern today, what would be different?"**

> "The pattern you wrote yesterday is a hypothesis. Today's knowledge is the test."

## What Updating Can Do

| Action | When to Do It |
|--------|---------------|
| **Add connections** | Newer patterns exist that should link here |
| **Rewrite content** | Understanding evolved, prose should reflect it |
| **Sharpen the claim** | Title is too vague to be useful |
| **Split the pattern** | Multiple claims bundled together |
...
Fully reconsider a specific pattern against current knowledge.
...
Process patterns flagged as sparse by /health.

### /update --since Nd

Update all patterns not updated in N days.

**How to find candidates:**
```bash
# Find patterns not modified in 30 days
find patterns/ -name "*.md" -mtime +30 -type f
```

### /update --handoff [[pattern]]
...
### Phase 1: Understand the Pattern as It Exists

Read the target pattern completely. Understand:
...
- Which domain guides were updated
...
Use the same dual discovery pattern as /connect — domain guide exploration AND semantic search in parallel.

**Path 1: Domain Guide Exploration** — curated navigation

From the pattern's Domains footer, identify which domain guide(s) it belongs to:
- Read the relevant domain guide(s)
- What synthesis exists that might affect this pattern?
- What newer patterns in Core Ideas should this pattern reference?
- What tensions involve this pattern?
...
**Also check:**
- Backlinks — what patterns already reference this one? Do they suggest the target should cite back?

```bash
grep -rl '\[\[target pattern title\]\]' patterns/ --include="*.md"
```

**Key question:** What do I know today that I did not know when this pattern was written?
...
| Claim is too broad | Split into focused patterns |
...
Does this pattern make multiple claims that could stand alone?
- If the pattern connects to 5+ topics across different domains, it probably needs splitting
- If you would want to link to part of it but not all, it is a split candidate

### Phase 4: Evaluate Connections

**Backward connections (what this pattern should reference):**

For each newer pattern, ask:
- Does it extend this pattern's argument?
...
**Forward connections (what should reference this pattern):**

Check newer patterns that SHOULD link here but do not:
...
```markdown
## Update Proposal: [[target pattern]]

**Last modified:** YYYY-MM-DD
**Current knowledge evaluated:** N newer patterns, M backlinks
...
- [[newer pattern A]] — [relationship]: [specific reason]
- [[newer pattern B]] — [relationship]: [specific reason]

### Connections to Verify (other patterns should link here)

- [[pattern X]] might benefit from referencing this because...
...
The simplest action. Newer patterns exist that should be referenced.
...
```yaml
relevant_notes:
  - "[[newer pattern]] — extends this by adding temporal dimension"
```
...
One pattern became multiple ideas over time. Splitting creates focused, composable pieces.
...
2. Create new patterns for each claim
3. Each new pattern gets:
   - Focused title (the claim)
   - Own description
   - Relevant subset of content
   - Appropriate connections
4. Original pattern either:
...
# if tension exists
This argues [X]. But [[contradicting pattern]] argues [Y]. The tension remains unresolved — possibly [X] applies in context A while [Y] applies in context B.

# if significantly wrong
This pattern originally claimed [X]. Based on [[evidence]], the claim is revised: [new claim].
```
...
After changes, is the pattern better? More useful? More connected? More accurate?

If you cannot confidently say yes, do not make the change.

### Gate 3: Coherence Test

After changes, does the pattern still cohere as a single focused piece? Or did you accidentally make it broader?
...
- The pattern is accurate, well-connected, and recent — leave it alone
- The "improvement" would just be cosmetic rewording — do not churn
- The pattern is a historical record — these evolve through status changes, not rewrites
...
```markdown
## Update Complete: [[target pattern]]

### Changes Applied

| Type | Description |
|------|-------------|
| connection | added [[pattern A]] inline, [[pattern B]] to footer |
...
- Outgoing links: 3 -> 5
- This pattern now bridges [[domain A]] and [[domain B]]

### Cascade Recommendations

- [[related pattern]] might benefit from update (similar vintage)
- Domain guide [[topic]] should be updated to reflect changes
...
Successful updating:
- Pattern reflects current understanding, not historical understanding
- Claim is sharp enough to disagree with
- Connections exist to relevant newer content
- Pattern participates actively in the network
- Someone reading it today gets the best version

The test: **if this pattern were written today with everything you know, would it be meaningfully different?** If yes and you did not change it, updating failed.
...
- Split patterns into pieces too thin to stand alone
- Add connections without articulating why
- Rewrite voice/style (preserve the pattern's character)
...
Patterns written yesterday do not know about today. Patterns written with old understanding do not reflect new understanding. Without updating, the vault becomes a graveyard of outdated thinking that happens to be organized.

Updating is how knowledge stays alive. Not just connecting, but questioning, sharpening, splitting, rewriting. Every pattern is a hypothesis. Every update is a test.
...
Work Done:
- Older patterns updated: N
- Claim status: unchanged | sharpened | challenged | split
- Network effect: M new traversal paths

Files Modified:
- patterns/[older pattern 1].md (inline link added)
- patterns/[older pattern 2].md (footer connection added)
...
- Older patterns updated and why
- Claim status (unchanged/sharpened/challenged/split)
...
# NEXT_PHASE is the phase after update in phase_order (i.e., verify)
...
- **manual:** Output "Next: /verify [pattern]" — user decides when to proceed
