...
- --type [type]: process only tasks at a specific phase (extract, create, connect, update, verify, enrich)
...
|-------|---------------|---------|
| extract | /extract | Extract patterns from source material |
| create | (inline pattern creation) | Write the pattern file |
| enrich | /enrich | Add content to existing pattern |
| connect | /connect | Find connections, update domain guides |
| update | /update | Update older patterns with new connections |
...
```yaml
phase_order:
  claim: [create, connect, update, verify]
  enrichment: [enrich, connect, update, verify]
...
Phase distribution:
  Claims:       {create: N, connect: N, update: N, verify: N}
  Enrichments:  {enrich: N, connect: N, update: N, verify: N}
...
Create a pattern for this claim in patterns/[claim as sentence].md
Follow pattern design principles:
...
The task file specifies which existing pattern to enrich and what to add.
ONE PHASE ONLY. Do NOT run connect.
```

For **connect** phase:

**Build sibling list:** Query the queue for other claims in the same batch where `completed_phases` includes "create" (pattern already exists). Format as wiki links.
...
Use dual discovery: domain guide exploration AND semantic search.
Add inline links where genuine connections exist — including sibling claims listed above.
Update relevant domain guide with this pattern.
...
This is the BACKWARD pass. Find OLDER patterns AND sibling claims
that should reference this pattern but don't.
Add inline links FROM older patterns TO this pattern.
...
1. **Collect all pattern paths** from completed batch tasks. For each claim task with `status: "done"`, read the task file's `## Create` section to find the created pattern path.
...
Patterns created in this batch:
{list of ALL pattern titles + paths from completed batch tasks}

Verify sibling connections exist between batch patterns. Add any that were missed
because sibling patterns did not exist yet when the earlier claim's connect ran.
...
**Why two phases?** Workers have sibling awareness (claim titles in spawn prompt) and link proactively during connect/update. But timing means some sibling patterns may not exist yet during a worker's connect phase. Phase B runs a single cross-connect pass after all patterns exist.
...
If a sibling pattern exists in patterns/, link to it inline where the
...
pattern title, path, and claim ID. The lead needs this for cross-connect.
```
...
1. **Parse completion message** — extract the created pattern title and path (needed for Phase B)
2. **Log any learnings** from the worker's report
3. **Check for issues** — failures, skipped phases, resource conflicts

**Collect all created patterns** — maintain a list of `{pattern_title, pattern_path}` from worker completion messages. You need this for the cross-connect validation phase.
...
Patterns created in this batch:
{list of ALL newly created pattern titles with paths from Phase A}

Verify sibling connections exist between these patterns. Add any connections that
workers missed because sibling patterns did not exist yet when a worker's connect ran.
Check backward link gaps. Output RALPH HANDOFF block when done.",
  description = "cross-connect: batch {BATCH}"
)
```
...
Queue state:
  Pending: {count}
  Done: {count}
  Phase distribution: {create: N, connect: N, update: N, verify: N}
...
After each phase, the task file's corresponding section (Create, Connect, Update, Verify) should be filled. If empty after subagent completes, log warning.
