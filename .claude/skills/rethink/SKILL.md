...
Every rule in the context file, every workflow in a skill, every assumption baked into the architecture was a hypothesis at some point. Hypotheses need testing against reality. Observation notes in `ops/observations/` capture friction from actual use. Tension notes in `ops/tensions/` capture unresolved conflicts. Rethink first triages these individually (some become patterns, some become methodology updates, some get archived), then compares remaining evidence against what the system assumes and proposes changes when patterns emerge.
...
|-------------|---------|---------------|--------|
| PROMOTE | Reusable insight worth keeping as a permanent pattern | General principle across sessions. Would work as a pattern. Crystallized insight, not operational guidance. | Create pattern in patterns/, set observation `status: promoted`, add `promoted_to: [[title]]` |
...
- Tension reveals a genuine conflict between two patterns → PROMOTE (create a tension pattern or resolution pattern)
...
--=={ rethink — Triage }==--
  ...
  PROMOTE ([count])
    [filename] — [title] → proposed pattern title
    [filename] — [title] → proposed pattern title
...
**For PROMOTE items:**
1. Create pattern with prose-as-title in patterns/
2. Follow standard pattern schema: YAML frontmatter (description, type, created), body developing the insight, Topics footer linking to relevant domain guide(s)
3. The observation content becomes the seed for the pattern body — but develop it fully, do not just copy the observation
4. Update the observation: set `status: promoted`, add `promoted_to: [[pattern title]]`
...
2. **Group by referenced domain guides or system areas:** Extract wiki links and file references from observation bodies. 3+ observations referencing the same area = recurring theme.
...
1. **Specific file references** — not "update the context file" but "update CLAUDE.md, section 'Processing Pipeline', paragraph 3"
...
### Promoted Patterns Need Connections

If any observations were promoted to patterns:

```
  [count] patterns were promoted from observations.
  Run /connect on promoted patterns to find connections.
  Promoted: [list of pattern titles]
```

### Pipeline Queue Integration

If promoted items should enter the processing pipeline (queue-based systems):
- Add each promoted pattern to the queue with `current_phase: "connect"` (the pattern already exists, so skip create)
...
- Recommend /reseed over patching: "Drift signals suggest the system's fundamental configuration may need re-derivation, not incremental patching. Consider running /arscontexta:architect for a configuration review."
...
- Create patterns directly from observations without going through standard pipeline (PROMOTE adds to queue)
