---
name: verify
description: Combined verification — recite (description quality via cold-read prediction) + validate (schema compliance) + review (health checks). Use as a quality gate after creating patterns or as periodic maintenance. Triggers on "/verify", "/verify [pattern]", "verify pattern quality", "check pattern health".
user-invocable: true
...
|-------|-----------------------|
| deep | Full verification: cold-read prediction, complete schema check, exhaustive link verification, domain guide coverage, orphan risk analysis, content staleness detection, bundling analysis |
| standard | Balanced: cold-read prediction, schema check, link verification, domain guide coverage |
...
Parse immediately:
- If target contains a pattern name: verify that specific pattern
- If target contains `--handoff`: output RALPH HANDOFF block at end
- If target is "all" or "recent": verify recently created/modified patterns
- If target is empty: ask which pattern to verify
...
1. COMPLETE description quality test — cold-read the title + description,
   predict what the pattern contains, compare against actual content.
...
3. COMPLETE link verification — confirm ALL wiki links in the pattern
   resolve to existing files. A single dangling link FAILS.

4. COMPLETE domain guide integration — verify the pattern appears in at least
   one domain guide's Core Ideas section with a context phrase.
   A pattern with no domain guide mention FAILS.
...
The index freshness check prevents false retrieval failures on recently created patterns. If the index is stale, retrieval test results should be interpreted with that context.

### Step 1: RECITE (cold-read prediction test)

**CRITICAL: Do NOT read the full pattern yet. Only read frontmatter.**

This step tests whether the title + description alone enable an agent to predict the pattern's content. The cold-read constraint is the entire point — reading the pattern first contaminates the prediction.
...
- Core argument: what claim does this pattern make?
- Mechanism: what reasoning or evidence does it use?
- Scope: what boundaries does the argument have?
- Likely connections: what other patterns would it reference?

Write this prediction explicitly in your output. It must be specific enough to be wrong.

**3. Read full pattern content**

NOW read the complete pattern. Compare against your prediction.
...
| **1** | Failed — pattern argued something different than expected | FAIL |
...
- Tier 1 (preferred): `mcp__qmd__vector_search` with query = "[the pattern's description text]", collection = "patterns", limit = 10
- Tier 2 (CLI fallback): `qmd vsearch "[the pattern's description text]" --collection patterns -n 10`
...
Check where the pattern appears in results:
- Top 3: description works well for semantic retrieval
- Position 4-10: adequate but could improve
- Not in top 10: flag — description may not convey the pattern's meaning

**Why vector_search specifically:** Agents find patterns via semantic search during connect and update. Testing with keyword search tests the wrong retrieval method. Full hybrid search with LLM reranking compensates for weak descriptions — too lenient. vector_search tests real semantic findability without hiding bad descriptions behind reranking.
...
Read the template that applies to this pattern type. Determine the template by checking:
- Pattern location (e.g., patterns/ uses the standard pattern template)
...
| Domains footer or `domains` field | Must reference at least one domain guide | FAIL |
...
If the pattern has fields with enumerated values (type, category, status, etc.), check them against the template's `_schema.enums` block. Each invalid enum value produces a WARN.
...
| Links exist | Each referenced pattern must exist as a file | WARN |

**Domains format (FAIL if invalid):**

| Constraint | Check | Severity |
|------------|-------|----------|
| Format | Array of wiki links: `["[[domain guide]]"]` | FAIL |
| Links exist | Each domain guide must exist as a file | WARN |

**Composability (WARN if fails):**

| Check | Rule | Severity |
|-------|------|----------|
| Title test | Can you complete "This pattern argues that [title]"? | WARN |
| Specificity | Is the claim specific enough to disagree with? | WARN |

### Step 3: REVIEW (per-pattern health checks)

Run these 5 checks on the pattern:
...
**3. Domain guide connection**
- Pattern appears in at least one domain guide's Core Ideas section
- How to check: grep for `[[pattern title]]` in files that serve as domain guides
- The pattern's Domains footer references a valid domain guide
- A pattern with no domain guide mention is orphaned — FAIL

**4. Wiki link density**
- Count outgoing wiki links in the pattern body (not just frontmatter)
- Expected minimum: 2 outgoing links
- If < 2: flag as sparse — the pattern is not participating in the graph
- Sparse patterns should be routed to /connect for connection finding

**5. Link resolution**
- Scan ALL wiki links in the pattern — body, frontmatter `relevant_notes`, and Domains
...
- Count incoming links: grep for `[[pattern title]]` across all .md files
- If 0 incoming links: AT RISK — pattern exists but nothing references it
- If 1 incoming link: LOW RISK — single point of connection
- If 2+ incoming links: OK

**7. Content staleness detection**
- Read the pattern's content and assess whether claims still seem current
- Check if referenced concepts/tools/approaches have changed
- Flag anything that reads as potentially outdated

**8. Bundling analysis**
- Does the pattern make multiple distinct claims that could be separate patterns?
- Check: could you link to part of this pattern without dragging unrelated context?
...
- Missing Domains footer (if obvious which domain guide applies)

**Do NOT auto-fix (requires judgment):**
- Bundled patterns (splitting requires understanding the claims)
...
- Ambiguous domain guide assignment (when pattern could fit multiple)
...
VALIDATE:
  ...
  Domains format: [PASS/FAIL — detail]
  Optional fields: [PASS/WARN/N/A]
  Relevant Patterns: [PASS/WARN/N/A]
  Composability: [PASS/WARN]

REVIEW:
  Frontmatter: [PASS/FAIL]
  Description quality: [PASS/WARN]
  Domain guide connection: [PASS/FAIL — which guide]
...
recite tests whether the description enables retrieval. validate checks schema compliance. review checks graph health. all three operate on the same pattern, read the same frontmatter, and together answer one question: is this pattern ready?
...
> "the unit of verification is the pattern, not the check type."

## execution order matters

**Recite MUST run first.** The cold-read prediction test requires forming an honest prediction from title + description BEFORE reading the full pattern. If validate or review ran first (both read the full pattern), the prediction would be contaminated. Recite's constraint: predict first, read second.

**Index freshness runs before everything.** The retrieval test in recite depends on semantic search having current data. Without a freshness check, recently created patterns produce false retrieval failures that obscure actual description quality issues.

After recite reads the full pattern, validate and review can run in any order since they both need the full content.

## recite: description quality

the testing effect applied to vault quality. read only title + description, predict what the pattern argues, then check. if your prediction fails, the description fails.

**why this matters:** descriptions are the API of the vault. agents decide whether to load a pattern based on title + description. a misleading description causes two failure modes:
- **false positive:** agent reads the pattern expecting X, wastes context on Y
- **false negative:** agent skips the pattern because description doesn't signal relevance

both degrade the vault's value as a knowledge tool.

**retrieval test rationale:** agents find patterns via semantic search during connect and update. testing with BM25 keyword matching tests the wrong retrieval method. full hybrid search with LLM reranking compensates for weak descriptions — too lenient. vector_search tests real semantic findability without hiding bad descriptions.
...
| `domains` | Must exist, array of wiki links | FAIL |
...
| Composability | Title passes "This pattern argues that [title]" test | WARN |

**FAIL means fix needed. WARN is informational but worth addressing.**

**template discovery:** The skill reads the template for the pattern type to get its `_schema` block. If no template exists or no `_schema` block is found, fall back to the default checks above.

## review: per-pattern health

5 focused checks per pattern (not a full vault-wide audit):
...
3. **Domain guide connection** — appears in at least one domain guide
...
8. **Bundling** — does the pattern make multiple distinct claims?
...
| Missing domain guide | Review fails guide check | Add to appropriate domain guide or create Domains footer |
| Dangling links | Review fails link resolution | Remove link, create the target pattern, or fix the spelling |
| Sparse pattern | < 2 outgoing links | Route to /connect for connection finding |
| Schema drift | Enum values not in template | Update pattern to use valid values, or propose enum addition |

## batch mode (--all)

When verifying all patterns:

1. Discover all patterns in patterns/ directory
2. For each pattern, run the full verification pipeline
...
**Performance note:** In batch mode, the recite cold-read test runs honestly for each pattern. Do not "warm up" by reading multiple patterns first — each prediction must be genuinely cold.

## standalone invocation

### /verify [pattern]

Run all three checks on a specific pattern. Full detailed report.

### /verify --all

Comprehensive audit of all patterns in patterns/. Summary table + flagged failures.
...
Files Modified:
- patterns/[pattern].md (description improved, if applicable)
...
Review:
- Frontmatter: PASS
- Domain guide connection: PASS ([[architecture]])
...
**never:**
- read the pattern before forming the recite prediction (cold-read is the whole point)
