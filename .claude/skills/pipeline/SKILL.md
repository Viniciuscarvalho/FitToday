---
name: pipeline
description: End-to-end source processing -- seed, extract, process all patterns through connect/update/verify, archive. The full pipeline in one command. Triggers on "/pipeline", "/pipeline [file]", "process this end to end", "full pipeline".
version: "1.0"
...
Phase 1: /seed — create extract task, move source to archive
    |
    v
Phase 2: /extract (via /ralph) — extract patterns from source
    |
    v
Phase 3: /ralph (all claims) — create -> connect -> update -> verify
    |
    v
...
Process the extract task via /ralph. This spawns a subagent that runs /extract, extracting patterns from the source and creating task entries in the queue.
...
After completion, read the queue to count extracted patterns and enrichments:

Check how many pending tasks exist for this batch. The extract phase creates 1 queue entry per claim and 1 per enrichment.

Report:
```
$ Extracted: {N} patterns, {M} enrichments
  Processing {total_tasks} tasks through the pipeline...
```

**If zero patterns extracted:** Report the issue. For iOS/Swift sources, zero extraction is a bug — the source almost certainly contains extractable content. Ask the user whether to retry with different scope or skip.
...
This processes every claim through: create -> connect -> update -> verify. And every enrichment through: enrich -> connect -> update -> verify.
...
```
$ Processing pattern 1/{total}: {title}
  $ create... done
  $ connect... done (3 connections found)
  $ update... done (2 patterns updated)
  $ verify... done (PASS)
```
...
- List of created patterns with titles
...
Processing:
  Patterns created: {N}
  Existing patterns enriched: {M}
  Connections added: {C}
  Domain guides updated: {T}
  Older patterns updated via update: {R}
...
**Extract failure:** If /extract extracts zero patterns, report and stop. Do not proceed to an empty processing phase.
...
**Archive failure:** If archiving fails, the patterns are still created and connected. Only the organizational cleanup is missing — re-run /archive-batch manually.
...
- Show the user what was created (list of patterns)
