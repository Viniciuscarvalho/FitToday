...
```
mcp__qmd__search query="patterns from {source filename}" limit=5
```

Or via keyword search in the patterns/ directory:
```bash
grep -rl "{key terms from source title}" patterns/ 2>/dev/null | head -5
```
...
**Why move immediately:** All references (task files, patterns' Source footers) use the final archived path from the start. No path updates needed later. If it is in captures/, it is unclaimed. Claimed sources live in archive.
...
# Extract patterns from {source filename}
...
## Acceptance Criteria
- Extract iOS/Swift patterns across all 7 categories
- Duplicate check against patterns/ during extraction
...
Patterns will start at: {NEXT_CLAIM_START}
Pattern files will be: {SOURCE_BASENAME}-{NNN}.md (unique across vault)
Queue: updated with extract task

Next steps:
  /ralph 1 --batch {SOURCE_BASENAME}     (extract patterns)
...
- Pattern files: `{source-basename}-{NNN}.md`
- Summary: `{source-basename}-summary.md`
- Archive folder: `{date}-{source-basename}/`

Pattern numbers (NNN) are globally unique across all batches, ensuring every filename is unique vault-wide. This is required because wiki links resolve by filename, not path.

## Source Handling Patterns

**captures/ source (most common):**
```
captures/swift-concurrency-wwdc.md
    | /seed
    v
ops/queue/archive/2026-02-23-swift-concurrency-wwdc/swift-concurrency-wwdc.md  <- source moved here
ops/queue/swift-concurrency-wwdc.md                                             <- task file created
```

**Living doc (outside captures/):**
```
CLAUDE.md -> stays as CLAUDE.md (no move)
ops/queue/archive/2026-02-23-claude-md/           <- folder still created
