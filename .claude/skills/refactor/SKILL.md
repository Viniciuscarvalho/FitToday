...
Changing a dimension is not just editing a value in config.yaml. Each dimension affects multiple artifacts — skills, templates, context file sections, hooks, domain guide structure. Changing organization from "flat" to "hierarchical" means restructuring the patterns directory, updating domain guide templates, adjusting navigation references in the context file, and regenerating skills that reference folder structure.
...
**The relationship to /arscontexta:architect:** /arscontexta:architect RECOMMENDS changes. /refactor IMPLEMENTS them. /arscontexta:architect analyzes health and friction to propose dimension shifts. When those proposals are approved, /refactor ensures every affected artifact is updated consistently.
...
|--------|-------------------|-------------|
| **Granularity shift** | Pattern templates, extraction depth in /extract, processing skills, context file "Note Design" section | Template body length guidance, extraction granularity settings, composability test thresholds |
| **Organization shift** | Folder structure, domain guide hierarchy, context file "Folder Architecture" section, hub domain guide | Directory layout, domain guide tier count, navigation references |
...
| **Navigation shift** | Domain guide tier structure, hub domain guide, context file "Domain Guide" section, pattern Topics footers | Number of domain guide tiers, hub content, navigation instructions |
| **Maintenance shift** | /arscontexta:health threshold values, condition-based trigger settings, context file maintenance instructions | Check frequency conditions, stale pattern thresholds, reweave trigger conditions |
...
Artifact: [file path]
  ...
  Content impact: [does this affect existing patterns? how many?]
```

### Content Impact Assessment

Some changes affect only infrastructure (skills, templates, context file). Others affect existing content (patterns, domain guides):

| Change Type | Content Impact | Action Required |
|-------------|---------------|----------------|
| Template field addition | New patterns get field, old patterns don't | Add field to old patterns (schema migration) |
| Template field removal | Old patterns have unused field | Remove field from old patterns (optional) |
| Folder restructure | Patterns must move | Move files, update all wiki links |
| Domain guide tier change | Domain guide hierarchy restructured | Merge/split domain guides, update Topics footers |
| Enum value change | Old patterns have invalid values | Update values in affected patterns |

For content-impacting changes:
- Count affected patterns: `grep -rl '[old value]' patterns/*.md | wc -l`
...
Detected [N] dimension changes:
  ...
  [dimension]: [old] -> [new]
    ...
    Content impact: [N] existing patterns need [what]
    Risk: [low/medium/high]
...
2. Apply vocabulary transformation from derivation-manifest
...
| Processing | /extract, /connect, /update, /verify, /ralph, /pipeline |
| Linking | /connect, /update |
| Navigation | /connect (domain guide update logic) |
...
For schema changes:
1. Read affected templates from templates/
...
1. **Schema migration:** Add/remove/rename fields across patterns
   ```bash
   # Example: add a new required field to all patterns
...
4. **Domain guide restructuring:** Merge/split domain guides as needed
   - For splits: create sub-guides, move Core Ideas, update parent
   - For merges: combine Core Ideas, remove redundant domain guide, update patterns' Topics footers
...
| Schema compliance | Check all patterns against template _schema blocks | All required fields present, enum values valid |
| Domain guide hierarchy | Verify hub links to domain guides, domains link to topics | All tiers connected |
| Session orient | Simulate session start — can the context file be loaded? | No file-not-found errors |
| Three-space boundaries | Check patterns vs ops vs self boundaries | No content in wrong space |
...
--=={ refactor complete }==--
  ...
  Content migration:
    [N] patterns updated with new schema fields
    [N] wiki links updated
    [N] domain guides restructured
...
```
  ...
  creates a combination that will fail because [reason].
...
### Large Content Migration (100+ patterns)

For large migrations:
1. Show the count and estimated time
2. Offer batch processing: "This affects 150 patterns. Process all at once or in batches of 50?"
